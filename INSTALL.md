...
mkdir -p ~/.hermes/plugins && \
  curl -sL https://github.com/WonderInventions/hermes-roam/releases/download/v0.0.5/hermes-roam-plugin-v0.0.5.tar.gz \
  | tar -xz -C ~/.hermes/plugins && \
  hermes plugins enable roam
```

This downloads the plugin tarball attached to release `v0.0.5`,
extracts it to `~/.hermes/plugins/roam/`, and enables it. Replace
`v0.0.5` with whichever release tag you want from
...
You should see `roam | enabled | 0.0.5 | Roam ... | user`. If it says
